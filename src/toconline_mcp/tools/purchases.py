...
_DOC_TYPE = "commercial_purchases_documents"
_PAY_TYPE = "commercial_purchases_payments"


class PurchaseDocumentLine(BaseModel):
    product_id: str | None = Field(None, description="TOCOnline product id (optional).")
    ...
    expense_category_id: str | None = Field(None, description="Optional expense category id.")
...
def register(mcp: FastMCP, client: TocClient) -> None:
    ...
    async def list_purchase_documents(
        ...
        """List commercial purchase documents with pagination. All filters exact match."""
    ...
    async def create_purchase_document(
        document_type: Annotated[str, Field(description="Purchase document type code.")],
        ...
        """Create a draft purchase document with line items."""
        if not lines:
        ...
        envelope = build_resource_envelope(
            _DOC_TYPE,
            attributes,
            relationships={"supplier": ("suppliers", safe_supplier_id)},
        )
    ...
    async def list_purchase_payments(
        ...
        """List purchase payments with pagination."""
        filters: dict[str, Any] = {}
        if supplier_id:
        ...
        if date:
            filters["date"] = require_iso_date(date, "date")
        return await client.request(
            "GET", _PAYMENTS_PATH, params=build_list_params(page_size=page_size, filters=filters)
        )
